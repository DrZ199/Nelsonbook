#!/bin/bash
# Restart Upload Script
# This script restarts the batch upload with a larger batch size to speed up the process.
...
# Run the batch upload in the background with a larger batch size
nohup python batch_upload.py --batch-size 100 --delay-seconds 2 > upload_log_restart.txt 2>&1 &

# Get the process ID
PID=$!

echo "Upload process restarted with PID: $PID"
echo "You can check the progress with: tail -f upload_log_restart.txt"
