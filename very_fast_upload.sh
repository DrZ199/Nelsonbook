#!/bin/bash
# Very Fast Upload Script with 10,000 rows per batch
# This script runs the batch upload with a very large batch size to maximize upload speed.
...
# Run the batch upload in the background with a very large batch size
nohup python batch_upload.py --batch-size 10000 --delay-seconds 5 > upload_log_very_fast.txt 2>&1 &

# Get the process ID
PID=$!

echo "Very fast upload process started with PID: $PID"
echo "Using batch size: 10,000 rows per batch"
echo "Using delay: 5 seconds between batches"
echo "You can check the progress with: tail -f upload_log_very_fast.txt"
